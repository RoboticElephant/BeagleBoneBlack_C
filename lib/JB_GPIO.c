...
#define PIN_DIRECTION(p)    "/sys/class/gpio/gpio"#p"/direction"
#define PIN_VALUE(p)        "/sys/class/gpio/gpio"#p"/value"
...
int gpioExDirection(int pin, int direc)
{
	...
	if((fp = fopen(PIN_DIRECTION(pin), "rb+")) == NULL)		// Opens direction file
	{
        #ifdef DEBUG
...
int closeGPIO(int pin)
{
	...
	if((fp = fopen(PIN_DIRECTION(pin), "rb+")) == NULL)
	{
        #ifdef DEBUG
		printf("Cannot open direction file\n");
        #endif
...
int gpioReadDigPin(int pin, int *value)
{
	...
	if((fp = fopen(PIN_VALUE(pin), "rb+")) == NULL)
	{
        #ifdef DEBUG
